...
class _SetupScreenState extends State<SetupScreen> {
  ...
  Future<void> _finish() async {
    ...
    provider.updateSettings(provider.settings.copyWith(currency: _currency));
      ...
      // Upload before creating the account. If the device is offline, keep the
      // local path so the logo remains available on this device.
      var storedLogoPath = _logoPath;
      final uid = SupabaseSyncService.currentUserId;
      if (storedLogoPath != null && uid != null) {
        try {
          storedLogoPath = await SupabaseSyncService.uploadAccountLogo(
                  uid, storedLogoPath) ??
              storedLogoPath;
        } catch (_) {
          // Keep the local path when the upload cannot complete.
        }
      }
          ...
          children: [
            ...
            Padding(
              ...
              child: GestureDetector(
                onTap:
                    _isSaving ? null : (_currentPage < 2 ? _nextPage : _finish),
                child: Container(
                  height: 56,
                  decoration: BoxDecoration(
                    ...
                    ? Column(
                        ...
                        children: [
                          Icon(Icons.add_a_photo_rounded,
                              size: 26, color: AppTheme.brandPrimary),
