...
class AppProvider with ChangeNotifier {
    ...
    for (int i = 0; i < _recurringRules.length; i++) {
      ...
      while (!nextDate.isAfter(now) && safetyCounter < 24) {
        ...
        final tx = Transaction(
          ...
          daretId: rule.templateTransaction.daretId,
          isRecurring: true,
  ...
  void deleteTransaction(String id) {
    ...
    if (old.accountId != cashOnHandId) {
      ...
      if (ai != -1) {
        ...
        if (old.type == 'expense' ||
            ...
            old.type == 'daret_contribution') {
          bal += old.totalWithFees;
        } else if (old.type == 'income' ||
            old.type == 'lending_collection' ||
            old.type == 'daret_payout') {
          bal -= old.amount;
        }
    ...
    final adjustedDaret = _reverseLinkedDaretOnDelete(old);
    ...
    // Daret state is local-only (no Supabase table); persisted via _saveToLocal.
    if (adjustedDaret != null && kDebugMode) {
      debugPrint(
          '[AppProvider] Daret ${adjustedDaret.name}: payout watermark rolled back to ${adjustedDaret.lastPayoutMonthProcessed} after deleting a payout transaction.');
    }
  }
  ...
  /// Reverses the persisted daret state when a linked daret transaction is
  /// removed. For a deleted `daret_payout`, rolls the payout watermark back so
  /// the payout can be regenerated by [processDaretPayouts] if still due, and
  /// returns the updated daret (null if none affected). Mirrors
  /// [_adjustLinkedGoal]; the account balance is reversed by the caller's
  /// generic reversal path via the type lists.
  Daret? _reverseLinkedDaretOnDelete(Transaction transaction) {
    final daretId = transaction.daretId;
    if (daretId == null || transaction.type != 'daret_payout') return null;

    final idx = _darets.indexWhere((d) => d.id == daretId);
    if (idx == -1) return null;
    final daret = _darets[idx];
    if (daret.lastPayoutMonthProcessed <= 0) return null;

    // Roll the watermark back by one processed payout month so this payout is
    // eligible to regenerate. Uses the highest processed month below the
    // current watermark among scheduled payout months.
    final priorMonths = daret.payoutMonths
        .where((m) => m < daret.lastPayoutMonthProcessed)
        .toList()
      ..sort();
    final newWatermark = priorMonths.isNotEmpty ? priorMonths.last : 0;
    _darets[idx] = daret.copyWith(lastPayoutMonthProcessed: newWatermark);
    return _darets[idx];
  }
  ...
  ///
  /// Note: for active darets the monthly contribution is normally driven by the
  /// `daret_contrib_<id>` recurring rule via [processSubscriptions] (idempotent
  /// through nextDate advancement). This direct method remains for manual
  /// one-off contributions; each created transaction is tagged with [daretId].
  ...
  /// Processes daret payouts for every active daret, idempotently.
  ///
  /// Call site: app startup, alongside [processSubscriptions]. For each active
  /// daret, generates one `daret_payout` transaction per due-but-unprocessed
  /// payout month (see [Daret.pendingPayoutMonths]) and advances the persisted
  /// [Daret.lastPayoutMonthProcessed] so restarts never duplicate.
  void processDaretPayouts() {
    for (final daret in List<Daret>.from(_darets)) {
      if (!daret.isActive) continue;
      checkDaretPayout(daret.id);
    }
  }

  /// Generate any due daret payout transactions for a single daret, once each.
  void checkDaretPayout(String daretId) {
    final idx = _darets.indexWhere((d) => d.id == daretId);
    if (idx == -1) return;
    final daret = _darets[idx];

    final dueMonths = daret.pendingPayoutMonths;
    if (dueMonths.isEmpty) return;

    final uid = _userId;
    int highestProcessed = daret.lastPayoutMonthProcessed;
    ...
    for (final month in dueMonths) {
      final txn = Transaction(
        ...
        date: DateTime.now().toIso8601String(),
        note: 'Daret payout: ${daret.name} (Month $month)',
      ...
      if (daret.destinationAccountId != cashOnHandId) {
        ...
        if (ai != -1) {
          ...
          if (uid != null) SupabaseSyncService.saveAccount(uid, _accounts[ai]);
        }
      }

      if (uid != null) SupabaseSyncService.saveTransaction(uid, txn);
      if (month > highestProcessed) highestProcessed = month;
    }

    // Persist idempotency watermark.
    _darets[idx] =
        daret.copyWith(lastPayoutMonthProcessed: highestProcessed);
